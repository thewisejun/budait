...
session_start();
  ?>
<head>
<title> Purchase Bitcoin with Paypal Or Credit Card </title>
...
<div class="gridContainer">
      ...
      ?>
        <!-- SESSIONS BEING CREATED JEFF-->
        <?php

if (isset($_POST['submit'])) { 
  header("Refresh: 1; url=cart_updated.php");
        $name =  $_POST['name'];
        $bitcoin_address= $_POST['btc_address'];
        $dob = $_POST['dob'];
        $price_usd= $_POST['amount'];
        if ($price_usd < 20) {
            $fee = .1;
        } elseif ($price_usd > 20) {
            $fee=.09;
        ...
        $_SESSION["name"]=$name;
      $_SESSION["bitcoin_address"]=$bitcoin_address;
      $_SESSION["dob"]=$dob;
      $_SESSION["amount"]=$price_usd;

}
        ?>
        <h6>Payment Details</h6>
        <div class="inputCon" id="name" data-top="Name on Paypal">
          <input type="text" name= "name" required placeholder="Arjun Kunwar"/>
        </div>
        <div class="inputCon" id="cardNum" data-top="Bitcoin Address" title = "type in the card number without spaces">
          <input type="text" name="btc_address" requiredplaceholder="1F1tAaz5x1HUXrCNLbtMDqcw6o5GNn4xqX"/>
        </div>
       
        <div class="inputCon" id="validYear" data-top="DOB">
          <input type="text" required name="dob"placeholder="09/20"/>
        </div>
        <div class="inputCon" id="cvv" data-top="Amount USD">
          <input type="numbers" required  name="amount" placeholder="$10"/>
        </div>
        
        <!--<button type="submit" form="paymentForm " name="submit2" onclick="window.location.href = 'cart_updated.php';" value="click"/> Continue></button>-->
